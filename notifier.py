...
import os
import socket
import socks  # 👈 必须安装: pip install PySocks
...
class EmailNotifier:
    ...
    def _send(self, subject, body):
        ...
        # === 🔥 核心黑魔法：临时挂载代理 ===
        # 1. 保存原始的 socket 连接方式 (发完邮件后必须还原！)
        original_socket = socket.socket

        try:
            # 2. 智能判断：有端口则挂载，无端口则直连
            if self.proxy_port:
                port = int(self.proxy_port)
                print(f"🌍 [邮件] 使用代理发送 (127.0.0.1:{port})")

                # 设置 SOCKS5 代理 (兼容大多数梯子)
                socks.set_default_proxy(socks.SOCKS5, "127.0.0.1", port)
                # 替换标准 socket 为 socks socket
            ...
            if "Time out" in str(e) or "10060" in str(e):
                print("   👉 提示: 连接超时。如果身在内地，请检查 PROXY_PORT 是否正确。")

        finally:
            # === 🔥 还原现场 (至关重要) ===
            # 无论发送成功还是失败，必须把 socket 还原回去
            # 否则可能会影响 CCXT 或 AI 模块的 HTTP 连接
